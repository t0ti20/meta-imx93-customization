...
#
# NFS rootfs deploy is NOT done here. It needs real root ownership (setuid
# binaries like su/passwd/busybox.suid must stay root:root to actually grant
# root on the board), which means running as root -- but BitBake tasks run
# under pseudo, which re-injects itself (LD_PRELOAD) into every child
# process, so `sudo` invoked from a task sees falsified file ownership and
# refuses to run. Rather than extract the rootfs unprivileged (which silently
# breaks those setuid binaries), NFS deploy is a manual, ordinary-root step:
#
...
do_deploy_network() {
    ...
    bbplain "    NFS rootfs update (run manually, needs real root ownership):"
